<template>
  <div class="mx-auto max-w-2xl">
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold">Категории проектов</h1>
      ...
      <ul v-else class="divide-y divide-gray-100">
        ...
        >
          <div>
            ...
            <p class="text-xs text-gray-400">Порядок: {{ cat.sortOrder }}</p>
          </div>
          <div class="flex gap-2">
            <UButton size="sm" variant="ghost" icon="i-lucide-pencil" @click="openEdit(cat)" />
            <UButton size="sm" variant="ghost" color="error" icon="i-lucide-trash-2" :loading="deletingId === cat.id" @click="deleteCategory(cat.id)" />
          </div>
        </li>
      </ul>
    </UCard>
        ...
        <div class="w-[400px] rounded-xl bg-white p-6 shadow-2xl">
          ...
          <div class="space-y-3">
            <UFormField label="Название" class="w-full">
              <UInput v-model="form.title" class="w-full" placeholder="Название категории" />
            </UFormField>
          ...
          <div class="mt-4 flex justify-end gap-2">
            ...
            <UButton :loading="saving" :disabled="!form.title.trim()" @click="saveCategory">
...
const { data, pending, refresh } = useFetch('/api/admin/project-categories')
...
async function saveCategory() {
  ...
  try {
    if (editItem.value) {
      await $fetch(`/api/admin/project-categories/${editItem.value.id}`, { method: 'PUT', body: form.value })
    } else {
      await $fetch('/api/admin/project-categories', { method: 'POST', body: form.value })
    }
    await refresh()
...
async function deleteCategory(id) {
  if (!confirm('Удалить категорию? Все проекты этой категории также будут удалены.')) return
  deletingId.value = id
  try {
